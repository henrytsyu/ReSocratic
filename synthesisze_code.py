...
def run_code(code):
    ...
    try:
        output = subprocess.check_output(
            ["python", "-c", exec_code], stderr=subprocess.STDOUT, timeout=1.5
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--prompt_path", type=str, default="prompt/synthesis/synthesize_code.txt"
    )
    ...
    for i in range(start_index, len(data), args.chunk):
        ...
        dialog_chunk = [
            construct_dialog(prompt, line["nl_solution"]) for line in chunk_data_input
        ]

        # Use the pool to map the worker function to each line in the data list
        responses_chunk = list(
            tqdm(
                workers_pool.imap(worker_partial, dialog_chunk), total=len(dialog_chunk)
            )
        )
        ...
        responses_chunk = [
            match_response_code(response) for response in responses_chunk
        ]
