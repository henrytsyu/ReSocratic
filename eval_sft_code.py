...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gencode_path",
        type=str,
        default="eval_results/optcode_llama3_8b_instruct_sft.json",
    )
    parser.add_argument(
        "--output_path", type=str, default="eval_results/llama3_8b_instruct_sft.json"
    )
    ...
    resulting_params = SamplingParams(
        temperature=0, max_tokens=512, stop=["* ", "<|eot_id|>"]
    )
    llm = LLM(
    ...
    for eval_line, gencode_line in zip(eval_data, gencode_data):
        if "```python\n" in gencode_line["llm_code"]:
            ...
            llm_match_code = match_response_code(
                "```python\n" + gencode_line["llm_code"]
            )
        code_correct, llm_code_output = run_code(llm_match_code)

        numercial_line_content = (
            "```code_output\n{}\n```\n\n\n".format(llm_code_output) + numercial_content
        )

        source_dialog = DIALOG_PROMPT + [
            {"role": "user", "content": eval_line["question"]},
            {"role": "assistant", "content": gencode_line["llm_code"]},
            {"role": "user", "content": numercial_line_content},
        ]
        ...
        if (
            not code_correct
            or "The problem could not be solved to optimality" in llm_code_output
        ):
            line_correct = False
            llm_results_dict = {}
        else:
