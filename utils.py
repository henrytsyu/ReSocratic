...
def make_chat_request_deepseek(
    model_name,
    messages,
    n,
    max_tokens=2000,
    ...
    if parallel:
        ...
        response_text_list = [
            response.choices[0].message.content for response in response_list
        ]
...
def check_dialog(dialog):
    ...
    if all([msg["role"] == "user" for msg in user_dialog]) and all(
        [msg["role"] == "assistant" for msg in assistant_dialog],
    ):
        return True
    else:
        print(
...
def remove_duplicates(paragraphs, threshold=0.8):
    ...
    paragraphs = [
        ...
        if paragraph not in stopwords.words("english")
    ]
