...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        default="/data1/yangzhch6/projs/ReSocratic/data/test_v1.json",
    )
    parser.add_argument(
        "--output_path", type=str, default="eval_results/deepseek-v2-chat_fewshot2.json"
    )
    parser.add_argument(
        "--prompt_path",
        type=str,
        default="/data1/yangzhch6/projs/ReSocratic/prompt/solve/scip_fewshot2.txt",
    )
    ...
    for index in range(len(inference_results), len(eval_data)):
        ...
        if (
            not code_correct
            or "The problem could not be solved to optimality" in code_output
        ):
            line_correct = False
            llm_results_dict = {}
        else:
            few_shot_dialog.append(
                {
                    "role": "assistant",
            ...
            for line_query in line["results"]:
                ...
                few_shot_dialog_numercial.append(
                    {"role": "user", "content": numercial_query_k}
                )

                numercial_responses = make_chat_request_deepseek(
                    args.model_name,
                    few_shot_dialog_numercial,
                    1,
                    temperature=0,
                    sleep=1,
                    parallel=False,
