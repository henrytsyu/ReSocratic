...
def construct_dialog(prompt, scenario):
    dialog = [
        {
            "role": "system",
            "content": "You are a mathematical assistant. Now, you will be provided with an optimization scenario. Please follow the example to convert the given scenario to question.",
        },
        {
            "role": "user",
            "content": prompt + "\n```Scenario\n{}```\n\n\n".format(scenario),
...
def worker_deepseek_chat(dialog, model_name, temperature=0, sleep=1):
    responses = make_chat_request_deepseek(
        model_name, dialog, 1, temperature=temperature, sleep=sleep, parallel=False
    )
    return responses[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_name", type=str, default="deepseek-chat"
    )  # deepseek-v2 chat
    ...
    parser.add_argument(
        "--notable_prompt_path",
        type=str,
        default="prompt/synthesis/synthesize_notable_question.txt",
    )
    parser.add_argument(
        "--table_prompt_path",
        type=str,
        default="prompt/synthesis/synthesize_table_question.txt",
    )
    ...
    for i in range(start_index, len(data), args.chunk):
        ...
        worker_partial = partial(
            worker_deepseek_chat, model_name=args.model_name, temperature=0, sleep=1
        )

        # Construct the dialog list
        table_dialog_chunk = [
            construct_dialog(table_prompt, line["nl_solution"]) for line in chunk_data
        ]
        notable_dialog_chunk = [
            construct_dialog(notable_prompt, line["nl_solution"]) for line in chunk_data
        ]

        # Use the pool to map the worker function to each line in the data list
        responses_table_chunk = list(
            tqdm(
                workers_pool.imap(worker_partial, table_dialog_chunk),
                total=len(table_dialog_chunk),
            )
        )
        responses_notable_chunk = list(
            tqdm(
                workers_pool.imap(worker_partial, notable_dialog_chunk),
                total=len(notable_dialog_chunk),
            )
        )
        ...
        responses_table_chunk = [
            match_response_question(response) for response in responses_table_chunk
        ]
        responses_notable_chunk = [
            match_response_question(response) for response in responses_notable_chunk
        ]
