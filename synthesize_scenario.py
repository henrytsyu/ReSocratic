...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pool_path", type=str, default="prompt/synthesis/pool/linear.json"
    )
    parser.add_argument(
        "--instruction_path", type=str, default="prompt/synthesis/instruction_{}.txt"
    )
    parser.add_argument(
        "--scenario_path", type=str, default="prompt/synthesis/scenario_{}.txt"
    )

    parser.add_argument(
        "--synthesis_response_path", type=str, default="synthesis_data/response_{}.json"
    )
    parser.add_argument(
        "--synthesis_data_path", type=str, default="synthesis_data/data_{}.json"
    )
    parser.add_argument(
        "--synthesis_scenario_path",
        type=str,
        default="synthesis_data/scenarios_{}.json",
    )
    ...
    args.scenario_path = args.scenario_path.format(
        "non_linear" if non_linear else "linear"
    )
    ...
    if os.path.exists(args.synthesis_scenario_path):
        ...
        filtered_v_o_responses = remove_duplicates(
            v_o_responses, threshold=args.threshold
        )

        filtered_responses = []
        for response in valid_responses:
            if (
                response.split("## Generate Constraint-1:\n")[0].strip()
                in filtered_v_o_responses
            ):
