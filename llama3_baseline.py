...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_path",
        type=str,
        default="eval_results/Mistral-7B_instruct_fewshot.json",
    )
    ...
    sampling_params = SamplingParams(
        temperature=0, max_tokens=2048, stop=["<|eot_id|>"]
    )
    resulting_params = SamplingParams(
        temperature=0, max_tokens=500, stop=["<|eot_id|>", "*"]
    )
    llm = LLM(
        model=args.model_name_or_path, tensor_parallel_size=args.tensor_parallel_size
    )
    ...
    DIALOG_ZERO_SHOT_PROMPT = [
        {
            "role": "system",
            "content": "Please use python code to solve the given question.",
        },
    ]
    DIALOG_FEW_SHOT_PROMPT = [
        {
            "role": "system",
            "content": "Please follow the given examples and use python code to solve the given question.",
        },
    ...
    for i in range(0, len(eval_data), args.batch_size):
        ...
        for example in batch:
            source_dialogs_zero_shot.append(
                DIALOG_ZERO_SHOT_PROMPT
                + [
                    {
                        "role": "user",
        ...
        # acquire code
        llm_code_batch = [
            match_response_code(line.outputs[0].text.strip()) for line in llm_code_batch
        ]
        ...
        for dialog, dialog_zero_shot, code, code_output in zip(
            source_dialogs,
            source_dialogs_zero_shot,
            llm_code_batch,
            llm_code_execute_output_batch,
        ...
        numercial_turn = {
            "role": "user",
            "content": """Accoding to the code output, please give your final answer for the following query. (The answer should be boxed in '\\boxed{}', and only in numerical form, and round it to 5 decimal places, such as '\\boxed{27.00000}', '\\boxed{3.20000}', and '\\boxed{0.23334}').""",
        }
        numercial_query_batch = [
            dialog_to_text_llama3(dialog + [numercial_turn])
            for dialog in source_dialogs_zero_shot
        ]

        llm_output_results_dict = []
        llm_output_correct = []

        for numercial_query, eval_line, code_output in zip(
            numercial_query_batch, batch, llm_code_execute_output_batch
        ):
            print(Fore.YELLOW + code_output + Style.RESET_ALL)
            print("-" * 40)

            if code_output in [
                "The code runs over time limit.",
                ...
                for key in llm_results_dict:
                    ...
                    llm_results_dict[key] = match_numercial_value(
                        llm.generate([numercial_query_result], resulting_params)[0]
                        .outputs[0]
                        .text.strip()
                    )
                    print(
